...
public class PlayGame {
    ...
    public static int specialAbility(String choice, Profile player, Enemy e) throws InterruptedException {
	if (choice.equals("1")) 
	    Abilities.calculatedStrike(player, e);
	
	else if (choice.equals("2"))
	    Abilities.armCannon(player, e);

	else if (choice.equals("3"))
	    Abilities.stun(player, e);

	else {
	    typeMsg("Invalid option, please pick an ability from the list below", 1);
	    return 0;
	}
		...
		while (e.getHP() > 0) {
			...
			while ( true ) {
			    ...
			    // cast
			        ...
			        // USE Ability CLASS HERE TO ATTACK
				updateMsg("You have " + player.getEnergy() + 
					  "/" + player.getBaseEnergy() + " energy.", 1);
				
				do {

				    player_action = updatePrompt("Select an ability below\n" +
								 "1| Calculated Strike - Cost: 20\n" +
								 "2| Arm Cannon - Cost: 40\n" +
								 "3| Stun - Cost: 25", 2);
				
				} while (specialAbility(player_action, player, e) == 0);
